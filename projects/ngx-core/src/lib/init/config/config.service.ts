...
import { AppEnv, Config, Environment } from './config';
...
export class ConfigService {
  ...
  initializr(): Promise<any> {
    ...
    if (this.browser.isIE11() && this.store.recover(this.KEY_CONFIG)) {
      return new Promise((resolve: Function) => {
        this.setConfig(this.store.recover<Config>(this.KEY_CONFIG));
        resolve(config);
      });
    }
